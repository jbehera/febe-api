...
export async function saveVersion(req: Request, res: Response) {
  ...
  const nextSnapshotVersionName = await versionService.getNextSnapshotVersion(projectId, baseForSnapshot);
...
export async function publishVersion(req: Request, res: Response) {
  ...
  const { settingsId, projectId, projectName, schemaJson, transformedJson, description, incrementType } = req.body;
  const userSettings = await userSettingService.getByQuery(`ID:${settingsId}`);

  // 1. Determine the next formal (published) version name
  const nextFormalVersionName = await versionService.getNextFormalVersion(projectId, schemaJson, incrementType || 'minor'); // Default to minor if not provided
  ...
  const matchingDraft = versions.data.find(v => areJsonObjectsEqual(unescapeJson(v.schemaJson ?? ""), schemaJson, ['createdAt', 'updatedAt']));
  ...
  try {
    const response = await versionService.publish(process.env.SCHEMA_PUBLISH_URL, payload);
