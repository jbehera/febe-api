...
export const versionService = {
  ...
  async getNextSnapshotVersion(
    ...
    const currentBaseVersion = baseSchemaVersion.startsWith('v')
      ? baseSchemaVersion.substring(1)
      : baseSchemaVersion;
    ...
    if (
      existingSnapshot &&
      existingSnapshot.name.startsWith(currentBaseVersion) &&
      existingSnapshot.name.includes('-snapshot.')
    ) {
  ...
  async getNextFormalVersion(
    ...
    const currentPublishedSchemaJson = unescapeJson(
      latestPublished.schemaJson || ''
    );

    if (
      areJsonObjectsEqual(currentPublishedSchemaJson, incomingSchemaJson, [
        'createdAt',
        'updatedAt',
      ])
    ) {
      throw new AppError(
        'No schema changes detected to publish. The incoming schema is identical to the latest published version.',
        409
      );
    }
    ...
    const [
      ...
      currentMinor,
      currentPatch,
    ] = latestPublished.name.split('.').map(Number);
    const [nextMajor, nextMinor, nextPatch] = incrementVersion(
    ...
    if (
      incrementType === 'major' &&
      (nextMajor <= currentMajor || nextMinor > 0 || nextPatch > 0)
    ) {
      throw new AppError(
        'Invalid major increment. Next major version should be greater than current, with minor and patch reset to 0.',
        400
      );
    }
    if (
      incrementType === 'minor' &&
      (nextMajor !== currentMajor || nextMinor <= currentMinor || nextPatch > 0)
    ) {
      throw new AppError(
        'Invalid minor increment. Next minor version should be greater than current, with patch reset to 0.',
        400
      );
    }
    if (
      ...
      throw new AppError(
        'Invalid patch increment. Next patch version should be greater than current.',
        400
      );
    }
